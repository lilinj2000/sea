...
void MDSpiImpl::OnLoginResponse(
    bool bSuccess,
    const char* pReason) {
...
void MDSpiImpl::OnQuoteUpdated(
  ...
  DepthMarketData* data = util_->toDepthMarketData(
      pDepthQuoteData);

  service_->pushData(data);
}

void MDSpiImpl::OnSymbolRegisterResponse(
    EesEqsIntrumentType chInstrumentType,
...
void MDSpiImpl::OnSymbolUnregisterResponse(
    EesEqsIntrumentType chInstrumentType,
    const char* pSymbol, bool bSuccess) {
...
void MDSpiImpl::OnSymbolListResponse(
    EesEqsIntrumentType chInstrumentType,
