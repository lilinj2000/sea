...
MDServiceImpl::MDServiceImpl(
  ...
  if (options_->protocol == "tcp") {
    ...
    strncpy(multiInfo.m_mcIp,
            options_->multi_ip.data(),
            sizeof(multiInfo.m_mcIp));
    multiInfo.m_mcPort = options_->multi_port;
    strncpy(multiInfo.m_mcLoacalIp,
            options_->local_ip.data(),
            sizeof(multiInfo.m_mcLoacalIp));
    multiInfo.m_mcLocalPort = options_->local_port;
    strncpy(multiInfo.m_exchangeId,
            options_->exchange_id.data(),
            sizeof(multiInfo.m_exchangeId));
...
void MDServiceImpl::subMarketData(
...
void MDServiceImpl::unsubMarketData(
    const std::string& instru) {
